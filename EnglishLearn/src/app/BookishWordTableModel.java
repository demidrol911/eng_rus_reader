    ...
    public Object getValueAt(int rowIndex, int columnIndex) {
        ...
        switch (columnIndex) {
            case 0: if(wordEntry.getDictEntries().isEmpty()) return false;
                    else return wordEntry.getDictEntries().get(0).isLearn();
    ...
    public void setValueAt(Object aValue, int rowIndex, int columnIndex) {
        if(columnIndex == 0){
           ...
           for(DictionaryEntry dictEntry: wordEntry.getDictEntries()){
               dictEntry.setLearn((boolean)aValue);
           }
           //wordEntry.getDictEntries().setLearn((boolean)aValue);
           LanguageFrame.translator.learnWord(wordEntry);
        }
    }
